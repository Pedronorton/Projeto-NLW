...
const Points = () => {
    ...
    useEffect( () => {
        async function loadPosition(){
            const {status} = await Location.requestPermissionsAsync(); // vai mostrar se o usuário permitiu ou não acessar a localização
            if(status !== 'granted'){
                Alert.alert('Ooooops....', 'Precisamos da sua permissão para obter a posição')
                return;
            }
        ...
        <>
            <View style={styles.container}>
                ...
                <View style={styles.mapContainer}>
                    {initialPosition[0] !== 0 && ( // uma condicional ternária só um if, não tem o else então basta colocoar o &&
                        <MapView 
                        style={styles.map}
                        initialRegion={{
